...
<template>
  <div ref="rootRef" class="slidev-layout two-cols-header w-full h-full flex flex-col" :style="{ '--chapter-no': info.chapter, '--slide-no': info.relativeSlide }">
    <div class="col-span-2 mb-3">
      <slot />
    </div>
    <div class="grid grid-cols-2 gap-6 flex-1 min-h-0" :class="alignClass">
      <div class="col-left h-full flex flex-col min-h-0">
        <slot name="left" />
      </div>
      <div class="col-right h-full flex flex-col min-h-0">
        <slot name="right" />
      </div>
    </div>
  </div>
