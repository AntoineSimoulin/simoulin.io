...
<template>
  <div class="w-full mb-4">
    <div class="w-full bg-white border-2 border-black hover:-translate-y-0.5 hover:shadow-[3px_3px_0px_#000] transition-all duration-200 rounded-2xl p-5 sm:p-6 flex flex-col justify-between">
      <div>
        <div class="flex flex-wrap items-center justify-between gap-2 mb-2">
          <a target="_blank" :href="citation.fields.url" class="no-underline text-black hover:text-purple-600">
            <h2 class="text-xl font-black tracking-tight leading-snug">{{ citation.fields.title }}</h2>
          </a>
          <span class="shrink-0 bg-purple-100 text-black border border-black px-2.5 py-0.5 rounded-md text-xs font-bold shadow-[1px_1px_0px_#000]">
